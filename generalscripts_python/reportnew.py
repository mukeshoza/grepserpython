...
#    rows = cur.fetchone()
#    rows = rows+1
cur.execute("SELECT distinct(data) from public.{}".format(tablename1))
dupdata = cur.fetchall()
...
for lines in rows:
    ...
    tiermatch = re.search('\'tier\D+\sNone\}\]', str(lines))
    # print(tiermatch)
    if tiermatch != None:
        tiermissing.append(tiermatch)

for zips in zipappend:
    if zips == '' or zips == None:
...
dfspec = pd.DataFrame(specappend, columns=['summary Specialities'])
...
print('===================================')
print('\n')
print(tabulate([['Total Data Count', len(rows)], ['Total Duplicate Lines Count',
                                                 len(rows)-len(dupdata)], ['Total Unique Lines Count', len(dupdata)]], tablefmt='fancy_grid'))
print('\n')
print('========================= ADDRESSES =========================')
print('\n')
...
missingnettier = len(tiermissing)
totaltier = len(nettier)
tiernet = (missingnettier)/len(rows)*100
roundtier = round(tiernet, 3)
print(tabulate([['Total Network', len(rows)], ['Missing Network', netpercent, str(roundnet)+'(%)'], ['Total Tier', len(rows)],
                ['Missing Tier', missingnettier, str(roundtier)+'(%)']],
               [], 'fancy_grid'))
...
print(tabulate([['Total Provider', totalprovider, 'Percentage (%)'], ['Missing Accepting New Patient', len(acceptingnewpatient), peracceptnew],
                ...
                ['Missing First Name', len(fname), fnameper], ['Missing Middle Name', len(mname), mnameper]
                , ['Missing Last Name', len(lname), lnameper], ['Missing Unparsed Name', len(uname), unameper],
                ['Missing Gender', len(gen), missinggender], ['Gender (Male)', len(genmale), missingmale],
                ['Gender (Female)', len(genfemale), missingfemale], ['Missing License', len(lnum), licnum],
...
print('\n')
print('=============== SPECIALITIES ===============')
spec = len(missingspec)/(totalprovider)*100
specto = len(specappend)
specmis = len(missingspec)
roundspec = round(spec, 3)
print(tabulate([['Total', totalprovider], ['Missing', specmis, str(roundspec)+'(%)']],
               [], 'fancy_grid'))
...
cur.execute("select data->'group_affiliations' from public.{}".format(tablename1))
groupaff = cur.fetchall()
totalgroup = len(groupaff)
for affi in groupaff:
    regrp = re.sub('^\(\[\{\D\w+\D\D\s\'|\'\}\]\,\)$|\(\[\]\,\)', '', str(affi))
    grpall.append(regrp)
    regroup = re.search('\(\[\]\,\)', str(affi))
    if regroup != None:
        groupmissing.append(regroup)
missinggrp = (len(groupmissing))
pergroup = (missinggrp/totalgroup)*100
roundgroup = round(pergroup, 3)
print(tabulate([['Total', totalgroup], ['Missing', missinggrp, str(roundgroup)+'(%)']],
               [], 'fancy_grid'))
dfgroup = pd.DataFrame(grpall, columns=['summary Group Aff'])
print(dfgroup.describe())
print("==========================================")
print('\n')

hospmissing = []
hosall = []
print('========== HOSPITAL AFFILIATION ==========')
cur.execute("select data->'hospital_affiliations' from public.{}".format(tablename1))
hospaff = cur.fetchall()
totalhosp = len(hospaff)
for hosp in hospaff:
    resub = re.sub('^\(\[\{\D\w+\D\D\s\'|\'\}\]\,\)$|\(\[\]\,\)','', str(hosp))
    hosall.append(resub)
    rehosp = re.search('\(\[\]\,\)', str(hosp))
    if rehosp != None:
        hospmissing.append(rehosp)
missinghosp = (len(hospmissing))
perhosp = (missinghosp/totalhosp)*100
roundhosp = round(perhosp, 3)
print(tabulate([['Total', totalhosp], ['Missing', missinghosp, str(roundhosp)+'(%)']],
               [], 'fancy_grid'))

dfhosp = pd.DataFrame(hosall, columns=['summary Hospital Aff'])
print(dfhosp.describe())
print("==========================================")
print('\n')
conn.commit()
